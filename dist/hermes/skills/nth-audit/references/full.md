...
2. **Shared fetch phase.** Fetch the content ONCE here, not inside each agent.
   ...
   `SITE-AUDIT.json`. Every dispatched agent READS these files with the read_file tool; no
   agent issues its own web_extract. One network pass feeds all 15 agents and each sees
...
3. **Dispatch the group in parallel.** For the selected mode, launch every agent
   in that group with the delegate_task, one `delegate_task` call per agent, all in a single
...
Each mode dispatches a fixed set of `goal` names. `full` runs all 15.

| Mode | Agents dispatched (`goal`) |
|------|-------------------------------------|
...
Launch all agents for the selected mode with the delegate_task in ONE message, one
`delegate_task` call per agent, so the harness runs them concurrently rather than one
...
`rendered.html`, `styles.css`, `scripts.js`, `headers.json`, `robots.txt`) plus
`SITE-AUDIT.json`, which it reads with the read_file tool. inspect-agent-a11y,
...
If the delegate_task or the plugin's sub-agents are unavailable in the current
