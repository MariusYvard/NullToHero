...
If a `DESIGN.md` already exists, **do not silently overwrite it**. Show the user the existing file and STOP and call the clarify tool to clarify whether to refresh, overwrite, or merge.
...
The following require creative input that cannot be auto-extracted. Group them into one `clarify` interaction:
...
Group into one `clarify` interaction. Options must be concrete.
