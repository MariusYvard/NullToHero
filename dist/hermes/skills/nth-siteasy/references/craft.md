...
Start from the direction, not a component library. If there is no DIRECTION.md, run `/nth-siteasy concept` first (see [concept.md](concept.md)) so the build has a committed idea to honor, and [memorability.md](memorability.md) for the bar to clear.

Run /nth-siteasy shape, passing along whatever feature description the user provided.
...
If the user has already run /nth-siteasy shape and has a confirmed design brief, skip this step and use the existing brief.
...
```bash
node "${NTH_ROOT}/tools/siteasy/variety.mjs" .
```
...
```bash
node "${NTH_ROOT}/tools/audit/gate.mjs" <the built file> --min-score 90
```
...
This is the same gate `/nth-audit` publishes as a GitHub Action, running the same 42 deterministic checks on the same code path. It is not a second opinion, it is the first one that can be wrong out loud.
