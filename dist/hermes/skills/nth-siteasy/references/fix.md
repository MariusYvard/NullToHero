...
| No audit output at all | Run `/nth-audit checks [target]` first ([../../audit/references/checks.md](../../audit/references/checks.md)), then proceed. |
...
| 4 | Verify | Re-run `/nth-audit checks` with the SAME settings as the source audit | No regression; the targeted findings resolved |
...
- Never fix findings ad hoc from the report prose: always through the mapped
  ...
  own doctrine.
- Batches are per command, not per page: one `/nth-siteasy animate` pass fixes all
...
- Not a re-audit: this reference consumes audit output; producing it belongs
  to `/nth-audit` ([../../audit/references/full.md](../../audit/references/full.md)).
