...
---
<!-- Generated for Hermes Agent from null-to-hero/skills/siteasy/.
     Do not edit here. Edit the source and run tools/build-dist.mjs. -->

## Host notes

Invoke this skill with the `nth-siteasy` skill (say what you want and Hermes picks it, or load it explicitly with `skill_view`). Its commands are written `/nth-siteasy <command>` below.
...
Use the read_file tool to check for these files. If already read in this session, don't re-read.
...
1. Load the matching reference file(s) listed in the table above using the read_file tool
