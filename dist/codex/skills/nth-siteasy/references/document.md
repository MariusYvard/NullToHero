...
- The user just ran `/nth-siteasy setup` and needs the visual side documented.
- The skill noticed no `DESIGN.md` exists and nudged the user to create one.
- An existing `DESIGN.md` is stale (the design has drifted).
- Before a large redesign, to capture the current state as a reference.

If a `DESIGN.md` already exists, **do not silently overwrite it**. Show the user the existing file and STOP and call the clarifying-question tool to clarify whether to refresh, overwrite, or merge.
...
Decide by scanning first (Scan mode Step 1). If the scan finds no tokens, no component files, and no rendered site, offer seed mode — don't silently switch. `/nth-siteasy document --seed` forces seed mode regardless of code presence.
...
The following require creative input that cannot be auto-extracted. Group them into one `clarifying-question` interaction:
...
4. **Refresh the session cache.** Run `node "${NTH_ROOT}/skills/siteasy/scripts/load-context.mjs"` one final time so the newly-written DESIGN.md lands in conversation. Subsequent commands in this session will use the fresh version automatically without re-reading.
...
Before interviewing: "There's no existing visual system to scan. I'll ask five quick questions to seed a starter DESIGN.md. You can re-run `/nth-siteasy document` once there's code, to capture the real tokens and components. OK?"

If the user prefers to skip, stop. No file.

### Step 2: Five questions

Group into one `clarifying-question` interaction. Options must be concrete.
...
```markdown
<!-- SEED — re-run /nth-siteasy document once there's code to capture the actual tokens and components. -->
```
...
2. Tell the user: "Re-run `/nth-siteasy document` once you have some code. That pass will extract real tokens and generate the sidecar."
3. Run `node "${NTH_ROOT}/skills/siteasy/scripts/load-context.mjs"` once so the seed lands in conversation for the rest of the session.
