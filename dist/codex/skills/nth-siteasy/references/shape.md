...
**Output**: A design brief that can be handed off to /nth-siteasy build, or directly to /nth-siteasy for freeform implementation. When visual direction probes are used, the images are supporting artifacts, not the primary output.
...
This is a required interaction, not optional guidance. Ask these questions in conversation, adapting based on answers. Don't dump them all at once; have a natural dialogue. STOP and call the clarifying-question tool to clarify.
...
```bash
python3 "${NTH_ROOT}/tools/design-system/scripts/search.py" "<the brief in a few words>" --domain landing
```
...
Record the chosen shape in the brief and in the `build` line appended to LOG.md, since the rotation rule reads it (L-VARIETY-2 refuses a shape used in the last three builds). Run `node "${NTH_ROOT}/tools/siteasy/variety.mjs" .` before choosing rather than after.
...
STOP and call the clarifying-question tool to clarify. Ask for explicit confirmation of the brief before finishing. If the user disagrees with any part, revisit the relevant discovery questions. A shape run is incomplete until the brief is confirmed.

When confirmation cannot be obtained, the brief still ships, marked: list every `[ASSUMED]` decision in a block at the top, and open the build presentation with that list. What the Craft gate forbids is a brief nobody can question, not a brief answered on stated assumptions. A brief whose assumptions are visible gets corrected in one message; a run that stopped gets abandoned.

Once confirmed, the brief is complete. The user can now hand it to /nth-siteasy, or use it to guide any other implementation approach. (If the user wants the full discovery-then-build flow in one step, they should use /nth-siteasy build instead, which runs this command internally.)
