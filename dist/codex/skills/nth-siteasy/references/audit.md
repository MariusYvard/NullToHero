...
For a deeper design-quality pass, `/nth-siteasy audit` dispatches four specialist sub-agents concurrently, each scoped to one design dimension. Launch them with the spawn_agent tool in a single message (one `spawn_agent` call per agent, same turn) so they run in parallel rather than in sequence. Pass each agent the target plus any source already in context.

| Sub-agent (`agent_type`) | Dimension | Covers |
|---|---|---|
...
Each agent returns its dimension score and findings. Wait for all four, then reconcile them with the five inline technical dimensions below (accessibility, performance, theming, responsive, anti-patterns), which catch implementation defects the design agents do not. If the spawn_agent tool or plugin agents are unavailable in the current harness, fall back to running the inline diagnostic scan only, in sequence. Never skip a dimension silently. For a whole-site pass that also covers search visibility and front-end defects, use `/nth-audit`.
...
**Rules**: Only recommend commands from: /nth-siteasy adapt, /nth-siteasy amplify, /nth-siteasy animate, /nth-siteasy audit, /nth-siteasy charts, /nth-siteasy clarify, /nth-siteasy critique, /nth-siteasy delight, /nth-siteasy document, /nth-siteasy harden, /nth-siteasy layout, /nth-siteasy mobile, /nth-siteasy onboard, /nth-siteasy overdrive, /nth-siteasy shape, /nth-siteasy polish, /nth-siteasy simplify, /nth-siteasy typeset. Map findings to the most appropriate command. End with `/nth-siteasy polish` as the final step if any fixes were recommended.

After presenting the summary, tell the user:

> You can ask me to run these one at a time, all at once, or in any order you prefer.
>
> Re-run `/nth-siteasy audit` after fixes to see your score improve.
