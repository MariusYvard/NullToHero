...
1. **Fetch homepage** via fetch or Bash
...
The audit runs five specialist sub-agents concurrently, each scoped to one dimension. Launch them with the spawn_agent tool in a single message (one `spawn_agent` call per agent, all in the same turn) so they execute in parallel rather than one after another. Pass each agent the target URL plus any page HTML already fetched.
...
Each agent returns its dimension score and findings. Wait for all five to complete, then aggregate. If the spawn_agent tool or plugin agents are unavailable in the current harness, fall back to running the five dimension checklists inline, in sequence, using the same scoring weights. Never skip a dimension silently.
