...
Each agent returns its scored section. Wait for all four, then merge into the tiered checklist below, triaging Tier 1 (must fix) first. If the spawn_agent tool or plugin agents are unavailable in the current harness, run the tiered checklist inline instead. Never skip a class silently. For a whole-site pass that also covers search visibility and design quality, use `/nth-audit`.
