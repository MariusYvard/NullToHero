...
description: >
  ...
  Backs the full, seo, defects, design, quick and verify run modes of /nth-audit.
version: 1.14.0
---

# Complete Site Audit

This is the orchestration playbook for every run mode of /nth-audit (`full`, `seo`,
...
2. **Shared fetch phase.** Fetch the content ONCE here, not inside each agent.
   ...
   come from one place:
   ```bash
   node ${NTH_ROOT}/tools/audit/fetch.mjs <url> --render --robots --assets-dir ./audit-assets --out fetch.json
   node ${NTH_ROOT}/tools/audit/analyze.mjs --from fetch.json --out SITE-AUDIT.json
   ```
   ...
   `SITE-AUDIT.json`. Every dispatched agent READS these files with the file reader; no
   agent issues its own fetch. One network pass feeds all 15 agents and each sees
...
3. **Dispatch the group in parallel.** For the selected mode, launch every agent
   in that group with the spawn_agent tool, one `spawn_agent` call per agent, all in a single
...
Each mode dispatches a fixed set of `agent_type` names. `full` runs all 15.

| Mode | Agents dispatched (`agent_type`) |
|------|-------------------------------------|
...
Launch all agents for the selected mode with the spawn_agent tool in ONE message, one
`spawn_agent` call per agent, so the harness runs them concurrently rather than one
...
`rendered.html`, `styles.css`, `scripts.js`, `headers.json`, `robots.txt`) plus
`SITE-AUDIT.json`, which it reads with the file reader. inspect-agent-a11y,
...
If the spawn_agent tool or the plugin's sub-agents are unavailable in the current
...
These five weights are specific to `/nth-audit`. `/nth-seo audit` scores the same site
...
executive summary:
```bash
node ${NTH_ROOT}/tools/audit/compare.mjs SITE-AUDIT.prev.json SITE-AUDIT.json --md
```
...
```bash
node ${NTH_ROOT}/tools/audit/reaudit.mjs --prev SITE-AUDIT.json --from fetch.json --out reaudit-plan.json
```
...
plugin's rules mid-audit; `/nth-audit learnings` reviews the accumulated candidates.
...
6. **Cost ledger.** Agents launched, approximate tokens and elapsed time, so cost is
   ...
   ```bash
   node ${NTH_ROOT}/tools/audit/cost.mjs --mode <mode> --html-bytes <bytes> --elapsed-ms <ms> --md
   ```
...
| Deterministic front-end defect scan | /nth-inspect detect |
