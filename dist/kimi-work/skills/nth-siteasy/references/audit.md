...
For a deeper design-quality pass, `/nth-siteasy audit` dispatches four specialist sub-agents concurrently, each scoped to one design dimension. Launch them with the Agent tool in a single message (one `Agent` call per agent, same turn) so they run in parallel rather than in sequence. Pass each agent the target plus any source already in context.
