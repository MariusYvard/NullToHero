...
to `/nth-seo drift compare` if a deploy landed in the window. Category A gets recorded and
...
buys nothing, and `/nth-seo schema` will flag it.
...
| The AI Overview cites the site but the page it cites is the wrong one | This is a canonical or an internal-linking problem, not a content one. Route to `/nth-seo technical` and the indexing grid in [search-console.md](search-console.md). |
...
| Crawler access, snippet directives, generative AI opt-in | `/nth-seo geo` |
| Rendering, status codes, canonical | `/nth-seo technical` |
| Schema validation before adding markup | `/nth-seo schema` |
