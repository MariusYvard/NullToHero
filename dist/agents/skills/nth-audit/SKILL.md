...
---
<!-- Generated for Agent Skills (open standard) from null-to-hero/skills/audit/.
     Do not edit here. Edit the source and run tools/build-dist.mjs. -->

## Host notes

Invoke this skill with `nth-audit`, or by asking for it by name. Its commands are written `/nth-audit <command>` below.
...
1. Read the matching reference file with the file reader (`full.md` for the agent run modes, `checks.md` for the deterministic pre-pass, `compare.md` for compare, `report.md` for formatting).
