...
3. **Dispatch the group in parallel.** For the selected mode, launch every agent
   in that group with the sub-agent delegation, one sub-agent delegation call per agent, all in a single
...
Each mode dispatches a fixed set of `agent type` names. `full` runs all 15.

| Mode | Agents dispatched (`agent type`) |
|------|-------------------------------------|
...
Launch all agents for the selected mode with the sub-agent delegation in ONE message, one
sub-agent delegation call per agent, so the harness runs them concurrently rather than one
...
If the sub-agent delegation or the plugin's sub-agents are unavailable in the current
