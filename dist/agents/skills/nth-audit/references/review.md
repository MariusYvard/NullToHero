...
For a full pre-ship review, `/inspect review` dispatches four deterministic defect specialists concurrently, each scoped to one class of front-end defect. Launch them with the sub-agent delegation in a single message (one sub-agent delegation call per agent, same turn) so they run in parallel. Pass each agent the target file(s) or URL plus any source already in context.

| Sub-agent (`agent type`) | Class | Covers |
|---|---|---|
