...
2. **STOP and call the clarifying-question tool to clarify.** to present these directions and get the user's pick before writing any code. Explain trade-offs (browser support, performance cost, complexity).
...
Implementation doctrine for spring physics and the View Transitions API lives in [animation-engineering.md](animation-engineering.md) (loaded by `/nth-siteasy animate`); this toolkit lists when to reach for each capability.
