...
For a deeper design-quality pass, `/nth-siteasy audit` dispatches four specialist sub-agents concurrently, each scoped to one design dimension. Launch them with the sub-agent delegation in a single message (one sub-agent delegation call per agent, same turn) so they run in parallel rather than in sequence. Pass each agent the target plus any source already in context.

| Sub-agent (`agent type`) | Dimension | Covers |
|---|---|---|
