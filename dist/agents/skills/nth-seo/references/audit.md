...
The audit runs five specialist sub-agents concurrently, each scoped to one dimension. Launch them with the sub-agent delegation in a single message (one sub-agent delegation call per agent, all in the same turn) so they execute in parallel rather than one after another. Pass each agent the target URL plus any page HTML already fetched.
