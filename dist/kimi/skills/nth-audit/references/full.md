...
2. **Shared fetch phase.** Fetch the content ONCE here, not inside each agent.
   ...
   agent issues its own FetchURL. One network pass feeds all 15 agents and each sees
...
3. **Dispatch the group in parallel.** For the selected mode, launch every agent
   in that group with the Agent tool, one `Agent` call per agent, all in a single
...
Launch all agents for the selected mode with the Agent tool in ONE message, one
`Agent` call per agent, so the harness runs them concurrently rather than one
...
If the Agent tool or the plugin's sub-agents are unavailable in the current
