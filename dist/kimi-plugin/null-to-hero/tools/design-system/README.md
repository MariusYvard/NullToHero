...
`--persist` writes a MASTER design system file plus optional per-page override files. This is the data-driven counterpart to `/nth-siteasy setup`, which is otherwise manual. Use the output to seed DESIGN.md, then refine by hand.
...
Each color pairing is checked against WCAG; a failing pair is flagged in a CSS comment rather than shipped. Tokens are an sRGB starter, refine the palette in OKLCH. This is the generative counterpart to the `/nth-siteasy tokens` audit.
