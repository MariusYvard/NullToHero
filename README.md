...
standalone Agent Skills in [`hermes-agent/`](hermes-agent/); `delegate_task` takes
a free-text goal per task built from each one's description. Install and keep
updated with:

```
bash install.sh --target hermes     # one-off, after a build
bash update-hermes-skills.sh        # build + convert + install, kept in sync
```

installs into `~/.hermes/skills/` (or `%LOCALAPPDATA%\hermes\skills\` on
Windows). Details in [`hermes-agent/README.md`](hermes-agent/README.md). As
with every other host, `null-to-hero/` is unaffected.
